...
# decoupe la phrase en mot puis va prend seulement se racine
...
def bow(sentence, words):
    # permet de normaliser la phrase
    sentence_words = clean_up_sentence(sentence)
    # creation d'un tableau de 0 en fonction de mot du corpus
    bag = [0]*len(words) 
    for s in sentence_words:
        for i,w in enumerate(words):
            if w == s:
                # si le mot de la phrase est dans ceux du corpus de mot il est note plus important
                bag[i] = 1
    return(np.array(bag))
